...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (!ClientPrefs.data.enableHBbgSupport) healthBar = new Bar(0, FlxG.height * (!ClientPrefs.data.downScroll ? 0.89 : 0.11), 'healthBar', function() return health, 0, 2);
		else healthBar = new Bar(0, FlxG.height * (!ClientPrefs.data.downScroll ? 0.89 : 0.11), 'healthBarTransparent', function() return health, 0, 2);
	...
	{
		// good note spawn times
		spawnTime = 1600 / songSpeed;
				...
				if(notes.length > 0)
				{
					if(startedCountdown)
					{
						...
						notes.forEachAlive(function(daNote:Note)
						{
							...
							else if (!daNote.hitByOpponent && !daNote.ignoreNote && (daNote.isSustainNote || daNote.strumTime <= Conductor.songPosition))
	...
	function set_health(value:Float):Float // You can alter how icon animations work here
	{
		...
		if (iconP1.animation.numFrames == 3) {
			...
			else if (healthBar.percent > 80 && !ClientPrefs.data.noWinningIcon)
		...
		if (iconP2.animation.numFrames == 3) {
			...
			else if (healthBar.percent < 20 && !ClientPrefs.data.noWinningIcon)
	...
	} else if (!ClientPrefs.data.noRatingWhenBotplay) {
		rating.loadGraphic(Paths.image(uiPrefix + daRating.image + uiSuffix));
		rating.screenCenter();
		rating.x = placement - 40;
		rating.y -= 60;
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		else if (!ClientPrefs.data.noStrumLight) strumPlayAnim(true, Std.int(Math.abs(note.noteData)), Conductor.stepCrochet * 1.25 / 1000 / playbackRate);
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		if(!cpuControlled)
		{
			...
			if(spr != null) {
				...
				else if (!ClientPrefs.data.noStrumLight) spr.playAnim('confirm', true);
			}
		}
		else {
			if (ClientPrefs.data.noStrumLight && !cpuControlled) strumPlayAnim(false, Std.int(Math.abs(note.noteData)), Conductor.stepCrochet * 1.25 / 1000 / playbackRate);
			else if (!ClientPrefs.data.noStrumLight) strumPlayAnim(false, Std.int(Math.abs(note.noteData)), Conductor.stepCrochet * 1.25 / 1000 / playbackRate);
		}
		vocals.volume = 1;
